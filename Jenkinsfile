      ...
      steps{
          ...
          withCredentials([usernamePassword(credentialsId:'docker-hub-repo',passwordVariable:'PASS',usernameVariable:'USER')])
        {
         // echo"remove the images step"
          ...
          sh"sshpass -p 'Ubuntu' ssh root@192.68.100.6 docker run -p 8085:8085  --network=mynet --name=timesheet${IMAGE_NAME} --link mysql:mysql -d  mokhtar194/timesheet:${IMAGE_NAME}"
        }
        }
